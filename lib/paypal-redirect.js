...
// =====================================================================
export async function createSubscriptionRedirect({ plan, clientSlug, returnUrl, cancelUrl, noTrial = false, env = process.env } = {}) {
  const isLive = (env.PAYPAL_ENV || 'live') === 'live';
  const base = isLive ? 'https://api-m.paypal.com' : 'https://api-m.sandbox.paypal.com';
  // Repeat signups (noTrial) use a no-trial plan if one is configured; otherwise
  // we safely fall back to the normal plan so nothing breaks before those plans exist.
  let planId;
  if (noTrial) {
    planId = (plan === 'pro')
      ? (env.PAYPAL_PRO_PLAN_NOTRIAL_ID || env.PAYPAL_PRO_PLAN_ID)
      : (env.PAYPAL_STARTER_PLAN_NOTRIAL_ID || env.PAYPAL_STARTER_PLAN_ID);
  } else {
    planId = (plan === 'pro') ? env.PAYPAL_PRO_PLAN_ID : env.PAYPAL_STARTER_PLAN_ID;
  }
